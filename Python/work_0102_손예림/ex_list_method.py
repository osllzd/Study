...
# [2] 리스트에 데이터 추가해주는 메서드 => insert( 위치 인덱스, 데이터) 지정 위치에 추가
nums.insert(0,2024)
print(f'nums:{len(nums)}개, {nums}')
...
nums.pop() # => 마지막 원소 7을 꺼냄
print(f'nums:{len(nums)}개, {nums}')

# 특정 위치에 원소/요소 데이터를 꺼내기 => pop(인덱스)
nums.pop(0) # -> 10을 꺼냄
print(f'nums:{len(nums)}개, {nums}')
...
# [8] 리스트를 확장시켜주는 메서드 => extend(여러개의 데이터를 저장한 데이터 타입)
